(function ($) {
  Drupal.behaviors.library_one_search = {
    	...
    	function search()
    	{
    		...
    		search =text[0].value;//At this point, all the textboxes will have the same search query, so thefore I will get from the 
    							//very first one.
 ...
 */
    		...
    		{    			
    			if("onesearch"===iAmOnThisPage)
    			{
    				...
    				"<span class=\"breadcrumb__page-title\">search</span>"; 
    				 ...
    				 document.getElementsByClassName("breadcrumb")[0].innerHTML =innerHTML;// innerHTML;
        		...
        		{
            		makeAllTextBoxTheSame(decodeURI(parseURLReturnSearch(window.location.protocol + "//"+window.location.host+"/"+location.pathname)));	
        		}
    	...
    	$(document).keypress(function(e) {
    	     ...
    	     */    	    	            		    	    	
    	    			...
    	    			window.location = window.location.protocol + "//"+window.location.host+"/onesearch/"+encodeURI(e.target.value);
      			 		return false;          			 
    	    }
    	});
    	...
    	$( ".form-submit" ).click(function()
    	{
    	    		...
    	    		!("search"==iAmOnThisPage && 2 ==curIdx))
    		{
        		...
        		{
        			window.location = window.location.protocol + "//"+window.location.host+"/onesearch/"+encodeURI(text[0].value);
        		}
        		else
        		{
        			window.location = window.location.protocol + "//"+window.location.host+"/onesearch/"+null;
        		}
